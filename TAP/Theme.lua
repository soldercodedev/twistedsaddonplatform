...
-- }
function UIF:NewTheme(opts)
    opts = opts or {}
    local theme = setmetatable({}, { __index = UIF.ThemeMixin })

    -- A skin (UIF.SKINS[name]) supplies palette + shape tokens; opts override it.
    local skin = opts.skin and UIF.SKINS and UIF.SKINS[opts.skin] or nil
    local C = {}
    for k, v in pairs(UIF.DEFAULT_PALETTE) do C[k] = { v[1], v[2], v[3] } end
    ...
    theme.id        = opts.name or UIF.NextId("UIFoundryTheme")
    theme.fonts     = opts.fonts or UIF.DEFAULT_FONTS   -- font registry (bundled set by default)
    -- Default asset dirs point at UIFoundry's own bundled art, so icons / shapes / fonts work
...
function Mixin:StylePanel(f, fill, edge, radius)
    local C = self.C
    fill, edge = UIF.toColor(fill, C.panel), UIF.toColor(edge, C.border)
    ...
    if shapeTex then
        ...
        f._rounded = false
        b:SetVertexColor(1, 1, 1); clearSlice(b); UIF.paint(b, edge)
        g:SetVertexColor(1, 1, 1); clearSlice(g); UIF.paint(g, fill)
    end
    return f
end
...
function Mixin:FillPaint(f, color)
    ...
    if f._rounded then local c = UIF.toColor(color, self.C.panel); f._fill:SetVertexColor(c[1], c[2], c[3], c[4] or 1)
    else UIF.paint(f._fill, UIF.toColor(color, self.C.panel)) end
end
function Mixin:EdgePaint(f, color)
    if not f._brd then return end
    if f._rounded then local c = UIF.toColor(color, self.C.border); f._brd:SetVertexColor(c[1], c[2], c[3], c[4] or 1)
    else UIF.paint(f._brd, UIF.toColor(color, self.C.border)) end
end
...
UIF.SHAPE_RADII = { 4, 6, 8, 10, 12, 16 }

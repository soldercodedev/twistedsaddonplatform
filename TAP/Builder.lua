...
local function acq(store, name, factory)
    ...
    w._tipTitle, w._tipBody, w._tipAnchor = nil, nil, nil   -- start each build without a stale tooltip
    w:Show(); return w
end
...
function BuilderMixin:Label(text, x, y, color, size)
    ...
    fs:SetFont(theme.FONT, size or 12); fs:SetJustifyH("LEFT"); fs:SetText(theme:HL(text))
...
function BuilderMixin:Section(text, x, y, w)
    ...
    d:ClearAllPoints(); d:SetPoint("TOPLEFT", self.content, "TOPLEFT", x, y - 18); d:SetWidth(w or (self.contentWidth - 108)); d:Show()
end
...
function BuilderMixin:Sub(text, x, y, w)
    ...
    d:ClearAllPoints(); d:SetPoint("TOPLEFT", self.content, "TOPLEFT", x, y - 15); d:SetWidth(w or (self.contentWidth - 48)); d:Show()
end
...
function BuilderMixin:Heading(text, x, y, role, opts)
    ...
    fs:SetWordWrap(opts.wrapWidth ~= nil); if opts.wrapWidth then fs:SetWidth(opts.wrapWidth) end
